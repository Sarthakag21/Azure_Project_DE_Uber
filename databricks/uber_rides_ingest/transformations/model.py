...
def dim_driver_view():
    df = spark.readStream.table("silver_obt")
    df = df.select("driver_id", "driver_name", "driver_rating", "passenger_phone", "driver_license")
...
dp.create_auto_cdc_flow(
  target = "fact",
  source = "fact_view",
  keys = ["passenger_id", "driver_id", "vehicle_id", "payment_method_id", "ride_id", "pickup_city_id"],
  sequence_by = "ride_id",
  stored_as_scd_type = 1
)