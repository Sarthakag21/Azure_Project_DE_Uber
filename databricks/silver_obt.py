...
df_schema = spark.sql("select * from uber.bronze.bulk_rides")
rides_schema = df_schema.schema
...
# MAGIC %sql
# MAGIC select fact.ride_id, dim.pickup_city, fact.base_fare from uber.bronze.fact fact left join uber.bronze.dim_location_view dim on fact.pickup_city_id = dim.pickup_city_id